...
automatic cycle-detection stage. A fair-gap robustness check (Section~\ref{sec:disc-cycle-source}) confirms this gap is not a single-split artefact, and a permutation test did not reach statistical significance ($p = 0.2475$).
...
macro-F1. A 5-fold fair-gap robustness check and a permutation test ($p = 0.2475$) both confirm the loss is not a single-split artefact (Section~\ref{sec:disc-cycle-source}). Since the comparison keeps the landmark-representation family fixed while changing the cycle source, the loss is attributed primarily to automatic cycle detection and boundary alignment.
