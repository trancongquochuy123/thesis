...
%   \usepackage{tikz}  \usetikzlibrary{positioning, arrows.meta, fit}
%
...
\node[process] (video) {Raw Running\\Video};
\node[process, below=of video] (pose) {Markerless Pose\\Estimation};
\node[process, below=of pose] (filter) {Landmark Filtering\\and Normalization};
\node[process, below=of filter] (signal) {Motion Signal\\Extraction};
\node[process, below=of signal] (cycle) {Running Cycle\\Detection};
\node[process, below=of cycle] (phase) {Eight-Phase\\Cycle Sampling};
\node[process, below left=1.15cm and 1.5cm of phase] (landmark) {Landmark-Based\\Representation};
\node[process, below right=1.15cm and 1.5cm of phase] (image) {Image-Based\\Representation};
\node[process, below=2.0cm of phase] (classifier) {Deep Learning\\Classifier};
\node[process, below=of classifier] (eval) {Evaluation and\\Error Analysis};

\draw[arrow] (video) -- (pose);
\draw[arrow] (pose) -- (filter);
\draw[arrow] (filter) -- (signal);
\draw[arrow] (signal) -- (cycle);
\draw[arrow] (cycle) -- (phase);
\draw[arrow] (phase) -- (landmark);
\draw[arrow] (phase) -- (image);
\draw[arrow] (landmark) -- (classifier);
\draw[arrow] (image) -- (classifier);
\draw[arrow] (classifier) -- (eval);
...
\node[
    ...
    fit=(landmark)(image),
    inner sep=0.25cm,
    label={[font=\small]above:Representation}
] {};

\end{tikzpicture}
\caption{Overall framework for vision-based running technique classification from in-the-wild videos. The pipeline first extracts markerless pose landmarks, filters and normalizes them, derives motion signals for running cycle detection, samples each cycle into eight phases, and then performs classification using either landmark-based or image-based representations.}
...
detection to stabilise period estimation.
