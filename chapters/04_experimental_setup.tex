...
reports configuration information that could be verified from the experiment
notebooks and the hyper-parameter evidence audit. Values are reported only when
they were explicitly found in the provided files; unsupported settings are still
marked as not exported rather than inferred. The audited landmark-sequence
experiments share a common training schedule of learning rate $10^{-3}$, batch
size 16, 80 epochs, weight decay $10^{-4}$, and random seed 42. The audited
image-based MobileNetV2--LSTM experiment uses learning rate $10^{-3}$, batch
size 8, 40 epochs, dropout 0.4, two recurrent layers, weight decay $10^{-4}$,
and random seed 42. Optimiser and loss-function details were not consistently
exported for the landmark notebooks, so they are not used as comparison factors.


\begin{table}[htbp]
\centering
\caption{Model configuration evidence available from the provided experiment files. Parameter counts are reported only when they are explicitly available in the exported model-ablation report. Training settings are reported only when they were explicitly found in the notebooks or hyper-parameter evidence audit.}
...
\toprule
Model & Pipeline use & Input representation & Parameters & Verified training settings \\
\midrule
MLP & Handmade and autocorr landmark model ablation & Eight-phase selected-landmark sequence with velocity features & $193{,}025$ & lr=$10^{-3}$; batch=16; epochs=80; weight decay=$10^{-4}$; seed=42. Optimiser/loss not exported consistently. \\
\addlinespace
1D-CNN & Handmade and autocorr landmark model ablation & Eight-phase selected-landmark sequence with velocity features & $40{,}257$ & lr=$10^{-3}$; batch=16; epochs=80; weight decay=$10^{-4}$; seed=42. Optimiser/loss not exported consistently. \\
\addlinespace
BiLSTM & Handmade and autocorr landmark model ablation & Eight-phase selected-landmark sequence with velocity features & $173{,}441$ & lr=$10^{-3}$; batch=16; epochs=80; weight decay=$10^{-4}$; seed=42. Number of recurrent layers is exported as two in the audited upper-bound notebook. \\
\addlinespace
CNN-BiLSTM & Handmade and autocorr landmark model ablation & Eight-phase selected-landmark sequence with velocity features & $82{,}113$ & lr=$10^{-3}$; batch=16; epochs=80; weight decay=$10^{-4}$; seed=42. Recurrent-layer details follow the model-ablation implementation evidence. \\
\addlinespace
MobileNetV2--LSTM & Autocorr image-based representation experiment & Eight-phase RGB runner crops, resized to $224\times224$ & Not exported & lr=$10^{-3}$; batch=8; epochs=40; weight decay=$10^{-4}$; dropout=0.4; recurrent layers=2; seed=42. Historical 76.5\% development code additionally used a two-phase AdamW/OneCycleLR schedule; details are listed in Appendix~\ref{app:hyperparams}. \\
\addlinespace
GRU / raw sequence model & Supplementary raw-sequence experiment & Video-level raw landmark sequence representation & Not exported & Not exported in the available hyper-parameter audit; treated as supplementary evidence only. \\
\bottomrule
\end{tabularx}
\end{table}
...
inventory. The table distinguishes between verified training settings and missing
implementation details. In particular, the image-based model has recoverable
learning-rate, batch-size, epoch, dropout, layer-count, and seed evidence, but no
exported parameter count. The raw-sequence model remains less fully documented
and is therefore interpreted only as supplementary evidence.
