...
Chapter~\ref{ch:literature-review} surveys the relevant literature and identifies the research gap.
Chapter~\ref{ch:methodology} describes the complete proposed framework, from pose estimation to classification and evaluation.
Chapter~\ref{ch:experimental-setup} specifies the experimental pipelines, training configuration, evaluation metrics, and the protocol separating controlled from historical results.
Chapter~\ref{ch:results} reports the empirical results across the controlled pipelines and ablation studies.
Chapter~\ref{ch:discussion} interprets these results, locating where performance is gained and lost.
Chapter~\ref{ch:conclusion} summarises the findings, answers the research questions, and outlines future work.
Throughout, conclusions are kept commensurate with the dataset and protocol used, and no claim of real-world deployment readiness is made.
