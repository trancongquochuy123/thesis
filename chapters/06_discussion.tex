...
differ chiefly in \emph{how cycle boundaries are obtained}. The fair-gap robustness check (Section~\ref{sec:disc-main}) confirms the gap is not a single-split artefact.
...
dimension also imports additional pose-estimation noise. 
% ---------------------------------------------------------------------
...
The permutation test provides a complementary diagnostic: the observed autocorr
MLP cross-validation macro-F1 of 0.5206 was not significant against a null
distribution obtained by label permutation ($p = 0.2475$, 100 permutations).
This does not prove the autocorr features carry no information, but it confirms
that any signal present is weak and unstable under a simple MLP --- consistent
with a pipeline constrained by cycle boundary quality rather than classifier
capacity.
...
of detecting clean running cycles in uncontrolled footage. 
% ---------------------------------------------------------------------
